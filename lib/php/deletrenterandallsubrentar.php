...
if (isset($_POST['renter_id'])) {
    ...
    $renter_id = filter_input(INPUT_POST, 'renter_id', FILTER_SANITIZE_NUMBER_INT);
    ...
    try {
        ...
        $sub_query = "DELETE FROM subtransaction WHERE renters_id = ?";
        $sub_stmt = $connect->prepare($sub_query);
        $sub_stmt->bind_param("i", $renter_id);
        $sub_stmt->execute();
        $sub_stmt->close();

        // Delete the renter record with the given renter_id
        $renter_query = "DELETE FROM renter WHERE id = ?";
        $renter_stmt = $connect->prepare($renter_query);
